...
def get_llm_instance():
    ...
    if deployment_env == "cloud":
        ...
        return VertexAILLM(
            ...
            project_id=os.getenv("GCP_PROJECT_ID", "anti-fraudx-us-ci")
        )
    else:
        # 本地部署：使用 Gemini API
...
# 容器鏡像
BACKEND_IMAGE=gcr.io/anti-fraudx-us-ci/anti-fraudx-backend
FRONTEND_IMAGE=gcr.io/anti-fraudx-us-ci/anti-fraudx-frontend
...
### 第一階段：準備（1-2 天）
- [ ] 創建 Google Cloud 項目 anti-fraudx-us-ci
- [ ] 啟用必要 API
- [ ] 設置 IAM 角色和服務帳戶
- [ ] 配置計費告警
...
```bash
# 創建項目
gcloud projects create anti-fraudx-us-ci --name="Anti-FraudX"
...
```bash
# 構建容器
gcloud builds submit --tag gcr.io/anti-fraudx-us-ci/anti-fraudx-backend

# 部署到 Cloud Run
gcloud run deploy anti-fraudx-backend \
  --image gcr.io/anti-fraudx-us-ci/anti-fraudx-backend \
  --platform managed \
  --region us-central1 \
  --memory 512Mi \
  --cpu 1 \
