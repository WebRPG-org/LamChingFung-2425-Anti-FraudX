        ...
        setupEvents: function() {
            ...
            this.recognition.onresult = function(event) {
                ...
                if (currentFinalTranscript) {
                    self.finalTranscript += currentFinalTranscript;
                }

                const displayText = self.finalTranscript + interimTranscript;
                console.log('[VoiceInput] 識別中:', displayText);
                
                // 設置2秒靜默計時器
                self.silenceTimer = setTimeout(function() {
                    if (self.recognition && self.isRecording) {
                        console.log('[VoiceInput] 2秒無語音，自動停止');
            ...
            this.recognition.onend = function() {
                ...
                // 調用回調並傳遞最終結果
                if (self.callback && self.finalTranscript) {
                    console.log('[VoiceInput] 最終結果:', self.finalTranscript);
                    self.callback(self.finalTranscript);
                }
