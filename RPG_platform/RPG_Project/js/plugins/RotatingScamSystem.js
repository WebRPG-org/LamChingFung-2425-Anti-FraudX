...
(function() {
    ...
    function rotateToNextScam(npcId) {
        ...
        console.log(`NPC${index+1} 切換到騙局${currentScam}: ${SCAM_TYPES[currentScam].name}`);
    }
    ...
    function startAIBattle(npcId, scam, isAutoMode) {
        ...
        .then(data => {
            ...
            console.log(`會話創建：${data.session_id}, 騙局：${scam.name}`);
    ...
    function generateVictimResponse(scam, callback) {
        ...
        fetch(`${API_URL}/chat`, {
            ...
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({
                role: `你正在與${scam.name}的騙徒對話。請根據你的人設自然回應。`,
                message: "請繼續對話",
    ...
    function generateScammerResponse(scam, victimMsg, callback) {
        ...
        fetch(`${API_URL}/chat`, {
            ...
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({
                role: `你是${scam.role}，正在進行${scam.name}。你的詐騙手法包括：${tactics}。請繼續誘騙對方。`,
                message: victimMsg,
                history: []
            })
        })
    ...
    function startAutoTalk(npcId) {
        ...
        interpreter.pluginCommand('StartScamBattle', [npcId.toString(), scamTypeId.toString()]);
    }
