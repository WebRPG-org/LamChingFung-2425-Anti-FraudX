...
if [ -n "$NVIDIA_VISIBLE_DEVICES" ]; then\n\
    ...
    echo "   ⚠️  NVIDIA_VISIBLE_DEVICES 未设置"\n\
fi\n\
\n\
if command -v nvidia-smi &> /dev/null; then\n\
    echo "   ✅ nvidia-smi 可用"\n\
    nvidia-smi --query-gpu=name,memory.total --format=csv,noheader 2>/dev/null || echo "   ⚠️  无法查询GPU信息"\n\
else\n\
    echo "   ⚠️  nvidia-smi 不可用（可能在CPU模式下运行）"\n\
fi\n\
echo ""\n\
\n\
# 等待Ollama服务就绪\n\
...
while [ $attempt -lt $max_attempts ]; do\n\
    if curl -sf ${OLLAMA_BASE_URL}/api/tags > /dev/null 2>&1; then\n\
        ...
        echo ""\n\
        echo "🔍 检查Ollama GPU状态..."\n\
        if curl -sf ${OLLAMA_BASE_URL}/api/ps 2>/dev/null | grep -q "models"; then\n\
