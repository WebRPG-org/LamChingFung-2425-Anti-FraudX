...
export class CollisionSystem {
    constructor(_scene, tilemap) {
        this.tilesetFlags = [];
        this.tileSize = 48;
        // Collision flag constants (RPG Maker MV standard)
        this.FLAG_IMPASSABLE = 0x0010; // 16 - 完全不可通行
        this.FLAG_DOWN = 0x0020; // 32 - 下方被阻擋
        this.FLAG_LEFT = 0x0040; // 64 - 左方被阻擋
        this.FLAG_RIGHT = 0x0080; // 128 - 右方被阻擋
        this.FLAG_UP = 0x0100; // 256 - 上方被阻擋
        this.FLAG_STAR = 0x0800; // 2048 - 星標可通行（從下方可通過）
    ...
    loadTilesetFlags() {
        ...
        const tilesetData = this.tilemap.getTileset(0);
        if (tilesetData && tilesetData.tileData) {
            const tileData = tilesetData.tileData;
            if (tileData.flags) {
     ...
     */
    getTileCollisionBounds(tileX, tileY) {
        if (!this.isValid(tileX, tileY)) {
            return null;
        }
        // Check all layers
        ...
        for (let i = 0; i < layers.length; i++) {
            ...
            if (tile) {
                ...
                if ((flags & this.FLAG_IMPASSABLE) !== 0) {
                    return new Phaser.Geom.Rectangle(tileX * this.tileSize, tileY * this.tileSize, this.tileSize, this.tileSize);
                }
            }
        }
        return null;
