...
export class AutoModeScene extends Phaser.Scene {
    ...
    create() {
        ...
        this.createControls();
        // Info text
        const info = this.add.text(width / 2, height - 60, '自動模式將持續運行模擬，生成訓練數據\n' +
    ...
    updateStatsDisplay(container, statBoxes) {
        statBoxes.forEach((stat, index) => {
            const valueText = container.getByName(`stat-${index}`);
            if (valueText) {
    ...
    runOneSimulation() {
        ...
        // Random duration
        this.stats.averageDuration = Math.floor((this.stats.averageDuration * (this.stats.totalSimulations - 1) +
