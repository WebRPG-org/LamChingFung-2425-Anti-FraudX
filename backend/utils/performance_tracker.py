...
from exceptions import MaxRoundsExceededError
...
class PerformanceTracker:
    ...
    def __init__(self, victim_persona: str = "elderly"):
        ...
        if not config.validate_persona(victim_persona):
            ...
            raise PersonaNotFoundError(
                victim_persona, 
                config.persona.AVAILABLE_PERSONAS
            )
        ...
        self.victim_trust.trust_in_expert = initial_trust["expert"]
        self.victim_trust.alertness = initial_trust["alertness"]
    
    def _get_max_trust_change_per_turn(self) -> int:
        """根据victim.py获取每轮最大信任度变化"""
        return config.get_max_trust_change(self.victim_persona)
    ...
    def _calculate_inertia_multiplier(self, current_trust: int, change: int) -> float:
        ...
        Returns:
            乘数 (0.5-1.0)
        """
        trust_cfg = config.trust
        
        if change > 0:
            # 正向变化：信任度越高，增加越难
            if current_trust >= 80:
                return trust_cfg.INERTIA_HIGH_TRUST_INCREASE
            elif current_trust >= 60:
                return trust_cfg.INERTIA_MID_TRUST_INCREASE
        else:
            # 负向变化：信任度越高，降低越难（更容易找借口）
            if current_trust >= 80:
                return trust_cfg.INERTIA_HIGH_TRUST_DECREASE
            elif current_trust >= 60:
                return trust_cfg.INERTIA_MID_TRUST_DECREASE
    ...
    def _calculate_fatigue_multiplier(self, tactics: List[str], recent_tactics: List[List[str]]) -> float:
        ...
        for tactic in tactics:
            ...
            if count >= 3:
                fatigue_penalties.append(trust_cfg.FATIGUE_3_TIMES)
            elif count >= 2:
                fatigue_penalties.append(trust_cfg.FATIGUE_2_TIMES)
            elif count >= 1:
                fatigue_penalties.append(trust_cfg.FATIGUE_1_TIME)
    ...
    def _calculate_emotional_multiplier(self, change: int, is_scammer: bool) -> float:
        ...
        if emotional_state == "anxious":
            return (trust_cfg.EMOTIONAL_ANXIOUS_SCAMMER_BOOST if is_scammer and change > 0 
                    else trust_cfg.EMOTIONAL_ANXIOUS_EXPERT_PENALTY)
        elif emotional_state == "calm":
            return (trust_cfg.EMOTIONAL_CALM_SCAMMER_PENALTY if is_scammer and change > 0 
                    else trust_cfg.EMOTIONAL_CALM_EXPERT_BOOST)
        elif emotional_state == "suspicious":
            return (trust_cfg.EMOTIONAL_SUSPICIOUS_SCAMMER_PENALTY if is_scammer and change > 0 
                    else trust_cfg.EMOTIONAL_SUSPICIOUS_EXPERT_BOOST)
        elif emotional_state == "panicked":
            return (trust_cfg.EMOTIONAL_PANICKED_SCAMMER_BOOST if is_scammer and change > 0 
                    else trust_cfg.EMOTIONAL_PANICKED_EXPERT_PENALTY)
    ...
    def update_trust(self, scammer_text: str, expert_text: str, victim_text: str) -> Dict:
        """更新信任度（整合三方分析）"""
        self.turn_count += 1
        
        # 分析骗徒表现
        scammer_analysis = self.analyze_scammer_turn(scammer_text, victim_text)
        
        # 分析专家表现
        expert_analysis = self.analyze_expert_turn(expert_text, victim_text, scammer_text)
        
        # 分析受害者回应
        victim_analysis = self.analyze_victim_response(victim_text, scammer_text, expert_text)
        ...
        return {
            ...
            "emotional_state": self.victim_trust.emotional_state,
            "scammer_change": scammer_analysis.get("trust_change", 0),
            "expert_change": expert_analysis.get("trust_change", 0),
            "scammer_tactics": scammer_analysis.get("tactics_used", []),
            "expert_approaches": expert_analysis.get("approach", [])
        }
    ...
    def check_outcome(self, conversation_history: List[Dict]) -> Dict:
        ...
        # 检查是否超过最大轮数
        if self.turn_count >= sim_cfg.MAX_ROUNDS:
            log.warning(f"⚠️ Simulation exceeded max rounds: {self.turn_count}/{sim_cfg.MAX_ROUNDS}")
        
        # 1. 骗徒成功（受害者高度信任骗徒）
        if self.victim_trust.trust_in_scammer >= trust_cfg.SCAMMER_WIN_THRESHOLD:
            self.identify_key_moment("scam_success", "受害者完全相信骗徒", self.turn_count)
            return {
                "status": "SUCCESS",
                "reason": f"受害者完全相信骗徒（信任度: {self.victim_trust.trust_in_scammer}），诈骗成功",
                "winner": "scammer"
            }
        
        # 2. 专家成功（受害者高度信任专家且不信任骗徒）
        if (self.victim_trust.trust_in_expert >= trust_cfg.EXPERT_WIN_THRESHOLD and 
            self.victim_trust.trust_in_scammer < trust_cfg.SCAMMER_LOSE_THRESHOLD):
            self.identify_key_moment("expert_success", "专家成功阻止诈骗", self.turn_count)
            return {
                "status": "FAILURE",
                "reason": f"专家成功说服受害者（专家信任: {self.victim_trust.trust_in_expert}, 骗徒信任: {self.victim_trust.trust_in_scammer}），诈骗失败",
                "winner": "expert"
            }
        
        # 3. 受害者高度警觉
        if self.victim_trust.alertness >= trust_cfg.HIGH_ALERTNESS_THRESHOLD:
            self.identify_key_moment("victim_alert", "受害者高度警觉", self.turn_count)
            return {
                "status": "FAILURE",
                "reason": f"受害者保持高度警觉（警觉度: {self.victim_trust.alertness}），诈骗失败",
                "winner": "victim"
            }
        ...
        if self.turn_count >= sim_cfg.MAX_ROUNDS:
            # 根据最终信任度判断
            if self.victim_trust.trust_in_scammer > self.victim_trust.trust_in_expert:
                return {
                    "status": "SUCCESS",
                    "reason": f"对话结束时受害者更信任骗徒（骗徒: {self.victim_trust.trust_in_scammer}, 专家: {self.victim_trust.trust_in_expert}）",
                    "winner": "scammer"
                }
            else:
                return {
                    "status": "FAILURE",
                    "reason": f"对话结束时受害者更信任专家（专家: {self.victim_trust.trust_in_expert}, 骗徒: {self.victim_trust.trust_in_scammer}）",
                    "winner": "expert"
                }
