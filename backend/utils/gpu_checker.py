...
class GPUChecker:
    ...
    def force_gpu_usage(self) -> bool:
        ...
        # 3. 检查Ollama GPU配置
        ollama_lib = os.getenv("OLLAMA_LLM_LIBRARY", "未设置")
        log.info(f"\n📚 OLLAMA_LLM_LIBRARY: {ollama_lib}")
        
        if ollama_lib != "cuda":
            log.error("❌ OLLAMA_LLM_LIBRARY 未设置为 'cuda'")
            log.error("   Ollama将使用CPU而不是GPU!")
            log.error("   请在docker-compose.yml中设置: OLLAMA_LLM_LIBRARY=cuda")
            
            if os.getenv("FORCE_GPU", "1") == "1":
                log.error("\n🚨 强制GPU模式启用，但配置不正确，启动失败！")
                log.error("   要禁用强制GPU模式，请设置环境变量: FORCE_GPU=0")
                sys.exit(1)
            else:
                log.warning("   继续启动，但会使用CPU（性能将显著下降）")
                return False
        ...
        # 6. 最终验证
        log.info("\n"+"="*80)
        if gpu_detected and ollama_lib == "cuda":
            log.info("✅ GPU配置正确，将使用GPU加速")
            log.info("="*80)
            return True
        else:
            log.warning("⚠️ GPU配置不完整，可能使用CPU")
            log.info("="*80)
            
            if os.getenv("FORCE_GPU", "1") == "1":
                log.error("\n🚨 强制GPU模式启用，但GPU未正确配置，启动失败！")
                log.error("   要禁用强制GPU模式，请设置环境变量: FORCE_GPU=0")
                sys.exit(1)
    ...
    def get_gpu_status_report(self) -> Dict:
        ...
        return {
            ...
            "environment": {
                ...
                "OLLAMA_LLM_LIBRARY": os.getenv("OLLAMA_LLM_LIBRARY"),
                "FORCE_GPU": os.getenv("FORCE_GPU", "1"),
            },
