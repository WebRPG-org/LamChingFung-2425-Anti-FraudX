...
class RealDialogueRunner:
    ...
    async def run_agent_with_adk(self, agent, message: str, session_id: str, conversation_history: list = None) -> str:
        """Run agent via Google ADK with conversation history support
        
        Args:
            agent: The agent to run
            message: Current message
            session_id: Session identifier
            conversation_history: List of previous messages [{"speaker": "...", "dialogue": "..."}]
        """
        ...
        if (agent_name.find('騙徒') != -1 and os.getenv('FORCE_DIRECT_SCAMMER', '0') == '1') or \
           (agent_name.find('受騙者') != -1 and os.getenv('FORCE_DIRECT_VICTIM', '0') == '1'):
            direct_only = await self._run_direct_llm(agent, message, conversation_history)
        ...
        if os.getenv("DISABLE_ADK", "0") == "1":
            direct = await self._run_direct_llm(agent, message, conversation_history)
        ...
        try:
            ...
            # Build ADK-compatible Content/Part with history
            # 🔥 新增：構建包含歷史的完整對話上下文
            contents = []
            
            # 添加對話歷史（如果有）
            if conversation_history:
                for msg in conversation_history[-10:]:  # 只取最近10輪，避免 token 過多
                    speaker = msg.get("speaker", "")
                    dialogue = msg.get("dialogue", "")
                    
                    # 判斷角色：如果是當前 agent 說的話，標記為 assistant；否則為 user
                    if speaker == agent_name:
                        role = "assistant"
                    else:
                        role = "user"
                        # 為了讓 agent 知道是誰說的，在對話前加上說話者
            ...
            contents.append(content_msg)
            
            # 如果沒有歷史，至少要有當前消息
            if not contents:
                contents = [content_msg]
            
            log.info(f"[ADK] agent={agent_name} 使用 {len(contents)} 個對話內容 (包含歷史)")
            
            # Note: ADK Runner.run() 只接受單個 new_message，不直接支持多消息
            # 但我們可以通過 session 來維護歷史（ADK 內部會處理）
    ...
    async def _run_direct_llm(self, agent, message: str, conversation_history: list = None) -> str | None:
        """Directly call agent-bound OllamaLlm with conversation history support
        ...
        try:
            ...
            # 添加對話歷史（如果有）
            if conversation_history:
                for msg in conversation_history[-10:]:  # 只取最近10輪
                    speaker = msg.get("speaker", "")
                    dialogue = msg.get("dialogue", "")
            ...
            log.info(f"[DIRECT] agent={agent_name} 使用 {len(contents)} 個對話內容 (包含歷史)")
        ...
        try:
            ...
            for turn in range(2, safety_cap + 1):
                ...
                expert_task = asyncio.create_task(self.run_agent_with_adk(
                    self.expert, expert_prompt, base_session + "_expert", conversation_history  # 🔥 傳遞對話歷史
                ))
                scammer_task = asyncio.create_task(self.run_agent_with_adk(
                    self.scammer, scammer_prompt, base_session + "_scammer", conversation_history  # 🔥 傳遞對話歷史
                ))
            ...
            recorder_response = await self.run_agent_with_adk(
                ...
                conversation_history  # 🔥 傳遞對話歷史（記錄員需要完整歷史）
            )
