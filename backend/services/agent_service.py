...
class AgentService:
    ...
    async def generate_response(
        ...
        try:
            ...
            # 3. 生成響應（使用 ADK Runner）
            response_text = await self._run_agent_with_runner(agent, prompt)
    ...
    async def _run_agent_with_runner(self, agent, message: str) -> str:
    ...
    def _check_consistency(
        ...
        try:
            ...
            if agent_type == "scammer":
                ...
                is_valid, checked, issues = RoleEnforcer.check_scammer_consistency(
                    dialogue=response,
                    previous_victim_message=previous_victim_msg
                )
                
                if not is_valid:
                    log.info(f"🔧 {agent_type.upper()} 響應已修正（一致性檢查）: {', '.join(issues)}")
                
                return checked
            
            elif agent_type == "expert":
                # check_expert_consistency(dialogue)
                is_valid, checked, issues = RoleEnforcer.check_expert_consistency(
                    dialogue=response
                )
                ...
                is_valid, checked, issues = RoleEnforcer.check_victim_consistency(
                    dialogue=response
                )
