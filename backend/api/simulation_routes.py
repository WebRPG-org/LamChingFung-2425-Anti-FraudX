...
async def _generate_finetuning_data(
    ...
    Args:
        ...
        analysis: RecorderAgent的分析結果
        performance_report: 性能報告
        metadata: 元數據（simulation_id, mode等）
        simulation_id: 模擬ID
    
    Returns:
        包含文件路徑的字典 {"expert_training": path, "scammer_training": path}
    """
    try:
...
async def run_simulation_async(simulation_id: str, victim_persona: str, scam_tactic: str, mode: str = "fast", auto_train: bool = True):
    ...
    try:
        ...
        # 移除語氣描述括號
        scammer_turn = remove_emotion_markers(scammer_turn)
        ...
        for turn in range(2, max_turns + 1):
            ...
            try:
                ...
                pass
            # 移除語氣描述括號
            victim_turn = remove_emotion_markers(victim_turn)
            ...
            if trust_in_scammer >= 100 or has_key_action:
                ...
                try:
                    ...
                    # 生成fine-tuning訓練數據
                    finetuning_files = await _generate_finetuning_data(
                        conversation_history=conversation_history,
                        analysis=analysis,
                        performance_report=final_report,
                    ...
                    await send_websocket_event(simulation_id, "simulation_end", {
                        ...
                        "analysis": analysis,
                        "filename": filename,
                        "finetuning_files": finetuning_files,  # 新增：fine-tuning文件路徑
                        "trust_final": {
                            "trust_in_scammer": trust_in_scammer,
                            "trust_in_expert": trust_in_expert,
            ...
            # 移除語氣描述括號
            expert_turn = remove_emotion_markers(expert_turn)
            ...
            # 專家本輪分析日誌（簡化版 - 詳細評分由 Recorder 提供）
            log.info(f"📊 [專家][第{turn}輪] 信任度變化 - 騙徒: {trust_in_scammer_after}/100, 專家: {trust_in_expert_after}/100")
            ...
            # 騙徒本輪分析日誌（簡化版 - 詳細評分由 Recorder 提供）
