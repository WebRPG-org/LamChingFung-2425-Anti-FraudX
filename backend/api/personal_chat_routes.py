...
async def send_message(
    ...
    try:
        ...
        # 處理上傳的圖片
        image_descriptions = []
        image_base64_list = []  # 儲存 base64 編碼的圖片
        ...
        if images:
            ...
            for idx, image in enumerate(images):
                if image and image.filename:
                    ...
                    # 轉換為 base64（Ollama 視覺 API 需要）
                    base64_image = base64.b64encode(content).decode('utf-8')
                    image_base64_list.append(base64_image)
                    
                    log.info(f"[個人對話] 圖片已上傳: {filename} ({len(content)} bytes)")
                    image_descriptions.append(f"[圖片 {idx+1}: {filename}]")
        ...
        log.info(f"[個人對話] {session_id} - 用戶: {full_message[:100]}...")
        ...
        if mode == "scammer":
            ...
            context_message = full_message
            # 如果有圖片，添加提示讓專家分析
            if image_descriptions:
                context_message += "\n\n（用戶上傳了圖片，請根據圖片內容判斷是否為詐騙）"
        
        # 如果有圖片，使用視覺服務（直接調用 Ollama API）
        if image_base64_list:
            log.info(f"[個人對話] 使用視覺服務分析 {len(image_base64_list)} 張圖片")
            
            # 獲取 ExpertAgent 的系統提示詞
        ...
        log.info(f"[個人對話] {session_id} - AI: {reply[:50]}...")
